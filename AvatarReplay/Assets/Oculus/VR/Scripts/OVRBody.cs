    ...
    OVRSkeletonRenderer.IOVRSkeletonRendererDataProvider
{
    ...
    public OVRController ovrBodyController;
    private bool isFromFile => ovrBodyController.isFromFile;
    ...
    private void Awake()
    {
        ...
        if (isFromFile)
        {
            try
            {
                stringArray = File.ReadAllLines(filePathBody);
            }
    ...
    public bool TryGetBodyState(out List<PosAndRot> posAndRots)
    {
        ...
        for (var i = 0; i < _bodyState.JointLocations.Length; i++)
        {
            ...
            var recordLocation = _bodyState.JointLocations[i];
    ...
    { 
        ...
        if (isFromFile)
        {
            ...
            {
                ovrBodyController.TrySetBody(indexOfFrame/*++*/, ref _bodyState);
            }
            else
            {
                //重新播放
