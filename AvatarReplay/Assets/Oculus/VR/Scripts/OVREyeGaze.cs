...
public class OVREyeGaze : MonoBehaviour
{
    ...
    public OVRController ovrEyeController;

    private bool isFromFile => ovrEyeController.isFromFile;
    ...
    private void Start()
    {
        ...
        if (isFromFile)
        {
            try
            {
                stringArray = File.ReadAllLines(filePathEyes);
            }
    ...
    private void Update()
    {
        ...
        if (ApplyPosition && !isFromFile)
        {
            transform.position = pose.position;
        }

        if (ApplyRotation && !isFromFile)
        {
        ...
        if (isFromFile)
        {
            if (indexOfFrame < stringArray.Length && (ApplyPosition || ApplyRotation))
            {
                Vector3 _vector = new(0, 0, 0);
                Quaternion _quaternion = new(0, 0, 0, 0);

                ovrEyeController.TrySetEyeGaze(indexOfFrame/*++*/, ApplyPosition, ApplyRotation, ref _vector, ref _quaternion, Eye);
                if (ApplyPosition)
                {
                ...
                if (ApplyRotation)
                {
                    transform.rotation = CalculateEyeRotation(_quaternion);
                }
    ...
    public bool TryGetEyeGazeState(EyeId eye,out Vector3 position,out Quaternion rotation)
    {
        ...
        {
            var pose = _currentEyeGazesState.EyeGazes[(int)Eye].Pose.ToOVRPose();
            switch (TrackingMode)
            {
                case EyeTrackingMode.HeadSpace:
                    pose = pose.ToHeadSpacePose();
            ...
            //Debug.Log("_currentEyeGazesState.EyeGazespos" + position.ToString());
