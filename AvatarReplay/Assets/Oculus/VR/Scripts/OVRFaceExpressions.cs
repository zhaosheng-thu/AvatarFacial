...
public class OVRFaceExpressions : MonoBehaviour/*, IReadOnlyCollection<float>*/
{
    ...
    public OVRController ovrFaceController;
    private bool isFromFile => ovrFaceController.isFromFile;
    ...
    /// changed
    /// </summary>
    public OVRPlugin.FaceState _currentFaceState;
    ...
    private void Awake()
    {
        ...
        indexOfFrame = 0;
        Debug.Log("filepath" + filePathFace);
        if (isFromFile)
        {
            try
            {
                stringArray = File.ReadAllLines(filePathFace);
            }
    ...
    private void Update()
    {
        ...
        if (isFromFile)
        {
            if (indexOfFrame < stringArray.Length)
            {
                ovrFaceController.TrtSetFaceExpression(indexOfFrame/*++*/, ref _currentFaceState);
            }
            else
            {
                //重新播放
    ...
    {
        //Debug.Log("IndexofFrame" + indexOfFrame);
        if (isFromFile)
        {
            if (indexOfFrame < stringArray.Length)
            {
